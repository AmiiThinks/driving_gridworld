...
def test_transition_probs_with_invisible_obstacle(obst, action):
    ...
    if action == LEFT or action == RIGHT:
        ...
        assert probs[1:] == [obst.prob_of_appearing / 2] * 2
        assert probs[0] == 1 - sum([obst.prob_of_appearing / 2] * 2)
...
def test_car_can_only_overdrive_headlights_by_one_unit():
    ...
    with pytest.raises(ValueError):
        patient = dg_road.LaneAndDitchRoad(headlight_range,
                                           Car(0, speed=headlight_range + 2))

    patient = dg_road.LaneAndDitchRoad(headlight_range,
                                       Car(0, headlight_range + 1))
...
def test_car_layer(col, headlight_range):
    patient = dg_road.LaneAndDitchRoad(headlight_range, Car(col, 1), [])
    x = np.full([headlight_range + 1, 5], False)
...
def test_wall_layer(headlight_range):
    patient = dg_road.LaneAndDitchRoad(headlight_range, Car(1, 1), [])
    x = np.full([headlight_range + 1, 5], False)
    x[:, 0] = True
    x[:, -2] = True
...
def test_obstacle_layers():
    ...
    x_bump_layer = np.full([headlight_range + 1, 5], False)
    x_bump_layer[0, 1] = True
    x_bump_layer[1, 2] = True
    np.testing.assert_array_equal(patient[str(bumps[0])], x_bump_layer)

    x_pedestrian_layer = np.full([headlight_range + 1, 5], False)
    x_pedestrian_layer[0, 2] = True
    x_pedestrian_layer[1, 1] = True
...
def test_board():
    ...
    headlight_range = 4
    speed = 1
    road = dg_road.LaneAndDitchRoad(headlight_range, Car(1, speed=speed),
                                    bumps + pedestrians)
    patient = road.board()
    ...
    x_board[1, 2] = bumps[0].to_byte()

    x_board[0, 2] = pedestrians[0].to_byte()
    x_board[1, 1] = pedestrians[0].to_byte()
...
def test_obstacles_appear_over_dirt_and_pavement(col):
    bump = Bump(row=0, col=col)
    headlight_range = 4
    patient = dg_road.LaneAndDitchRoad(headlight_range, Car(col=1, speed=1),
                                       [bump]).board()
    assert patient[0, col + 1] == bump.to_byte()
...
def test_car_has_crashed(col):
    patient = dg_road.LaneAndDitchRoad(4, Car(col, 1), [])
...
def test_successor_probabilities(speed):
    state = dg_road.LaneAndDitchRoad(headlight_range=6,
                                     car=Car(1, speed),
                                     obstacles=[Bump(-1, -1)])
...
def test_moving_car_appears_when_stopped():
    ...
    assert successors[0] == ((car_col, 0, frozenset()), 0.5)
    assert successors[1] == ((car_col, 0, frozenset([('p', 0, 1, 1, 0)])), 0.5)


def test_available_spaces():
    s = dg_road.LaneAndDitchRoad(headlight_range=3, car=Car(0, 0))
    assert s.available_spaces(2) == set(product(range(2), range(2)))
